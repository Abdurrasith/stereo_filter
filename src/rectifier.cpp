...
Rectifier::Rectifier(
		const Mat& m_l, const Mat& d_l, const Mat& r_l, const Mat& p_l,
		const Mat& m_r, const Mat& d_r, const Mat& r_r, const Mat& p_r,
	...
	//int width = 640;//config_l["image_width"].as<int>();
	//int height = 480;//config_l["image_height"].as<int>();
	...
	//cv::stereoRectify(
	//		m_l,d_l,m_r,d_r,Size(width,height),
	//		cv::Mat::eye(3, 3, CV_64F), T,
	//		r1,r2,p1,p2,Q);
...
void Rectifier::apply(const Mat& l, const Mat& r, Mat& rect_l, Mat& rect_r){
	cv::remap(l,rect_l,c_l_m1,c_l_m2,INTER_LINEAR); 
	cv::remap(r,rect_r,c_r_m1,c_r_m2,INTER_LINEAR); 
}
